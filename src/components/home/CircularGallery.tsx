    ...
    const program = new Program(this.gl, {
      ...
      fragment: `
        ...
        void main() {
          ...
          // Поріг 0.1 зрізав згладжені краї, і тонкі засічки читалися
          // напівпрозорими. 0.01 прибирає лише повністю порожні пікселі.
...
class Media {
  ...
  update(scroll: { current: number; last: number }, direction: 'right' | 'left', animate = true) {
...
class App {
  ...
  boundOnTouchDown!: (e: PointerEvent) => void;
  boundOnTouchMove!: (e: PointerEvent) => void;
  boundOnTouchUp!: () => void;
  ...
  update() {
    ...
    const moving = distance > 0.001 || this.isDown;

    if (!moving && !this.needsRender) return;

    this.scroll.current = lerp(this.scroll.current, this.scroll.target, this.scroll.ease);
    const direction = this.scroll.current > this.scroll.last ? 'right' : 'left';
    if (this.medias) {
      this.medias.forEach(media => media.update(this.scroll, direction, moving));
    }
  ...
  addEventListeners() {
    ...
    */
    ...
    this.container.addEventListener('pointermove', this.boundOnTouchMove as EventListener);
    this.container.addEventListener('pointerup', this.boundOnTouchUp);
    this.container.addEventListener('pointercancel', this.boundOnTouchUp);
  ...
  destroy() {
    ...
    this.container.removeEventListener('pointerdown', this.boundOnTouchDown as EventListener);
    this.container.removeEventListener('pointermove', this.boundOnTouchMove as EventListener);
