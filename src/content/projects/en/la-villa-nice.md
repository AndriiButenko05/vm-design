---
title: "La Villa"
city: "Nice"
country: "France"
type: residential
year: 2025
scope:
  - "Concept design"
  - "Space planning"
  - "Working drawings"
  - "Custom joinery"
  - "Author supervision"
materials:
  - "Brown onyx"
  - "Calacatta marble"
  - "Brass"
  - "Herringbone oak"
  - "Lacquered panelling"
summary: "A gutted villa above Nice, rebuilt around marble and brass."
cover: ../../../assets/projects/la-villa-nice/005-img-2451.jpg
coverAlt: "La Villa — marble, brass and lacquered joinery in the finished villa"
coords: { lat: 43.7102, lon: 7.262 }
featured: true
order: 1
todo: "Рік із назви PDF — підтвердити. Потрібні площа й авторський текст."
gallery:
  - src: ../../../assets/projects/la-villa-nice/007-img-2458.jpg
    alt: "La Villa — marble, brass and lacquered joinery in the finished villa"
    size: full
  - src: ../../../assets/projects/la-villa-nice/034-img-2560.jpg
    alt: "La Villa — marble, brass and lacquered joinery in the finished villa"
    size: half
  - src: ../../../assets/projects/la-villa-nice/028-img-2528.jpg
    alt: "La Villa — marble, brass and lacquered joinery in the finished villa"
    size: half
  - src: ../../../assets/projects/la-villa-nice/016-img-2485.jpg
    alt: "La Villa — marble, brass and lacquered joinery in the finished villa"
    size: half
  - src: ../../../assets/projects/la-villa-nice/043-img-2582.jpg
    alt: "La Villa — marble, brass and lacquered joinery in the finished villa"
    size: detail
    material: "Brown onyx"
  - src: ../../../assets/projects/la-villa-nice/025-img-2519.jpg
    alt: "La Villa — marble, brass and lacquered joinery in the finished villa"
    size: half
  - src: ../../../assets/projects/la-villa-nice/030-img-2530.jpg
    alt: "La Villa — marble, brass and lacquered joinery in the finished villa"
    size: half
  - src: ../../../assets/projects/la-villa-nice/022-img-2508.jpg
    alt: "La Villa — marble, brass and lacquered joinery in the finished villa"
    size: half
  - src: ../../../assets/projects/la-villa-nice/032-img-2553.jpg
    alt: "La Villa — marble, brass and lacquered joinery in the finished villa"
    size: half
  - src: ../../../assets/projects/la-villa-nice/020-img-2498.jpg
    alt: "La Villa — marble, brass and lacquered joinery in the finished villa"
    size: detail
    material: "Lacquered panelling"
  - src: ../../../assets/projects/la-villa-nice/021-img-2502.jpg
    alt: "La Villa — marble, brass and lacquered joinery in the finished villa"
    size: half
  - src: ../../../assets/projects/la-villa-nice/036-img-2564.jpg
    alt: "La Villa — marble, brass and lacquered joinery in the finished villa"
    size: half
  - src: ../../../assets/projects/la-villa-nice/038-img-2569.jpg
    alt: "La Villa — marble, brass and lacquered joinery in the finished villa"
    size: half
  - src: ../../../assets/projects/la-villa-nice/041-img-2572.jpg
    alt: "La Villa — marble, brass and lacquered joinery in the finished villa"
    size: half
  - src: ../../../assets/projects/la-villa-nice/042-img-2578.jpg
    alt: "La Villa — marble, brass and lacquered joinery in the finished villa"
    size: detail
    material: "Brass"
  - src: ../../../assets/projects/la-villa-nice/008-img-2461.jpg
    alt: "La Villa — marble, brass and lacquered joinery in the finished villa"
    size: half
beforeAfter:
  mode: slider
  pairs:
    - before: ../../../assets/projects/la-villa-nice/045-img-7562.jpg
      after: ../../../assets/projects/la-villa-nice/020-img-2498.jpg
      caption: "The stair hall: original stone flight and balustrade, rebuilt as a cantilevered oak stair"
  images:
    - src: ../../../assets/projects/la-villa-nice/044-img-7561.jpg
      alt: "La Villa — the villa stripped back to structure during the works"
    - src: ../../../assets/projects/la-villa-nice/045-img-7562.jpg
      alt: "La Villa — the villa stripped back to structure during the works"
    - src: ../../../assets/projects/la-villa-nice/046-img-7831.jpg
      alt: "La Villa — the villa stripped back to structure during the works"
    - src: ../../../assets/projects/la-villa-nice/050-img-8886.jpg
      alt: "La Villa — the villa stripped back to structure during the works"
    - src: ../../../assets/projects/la-villa-nice/051-img-8889.jpg
      alt: "La Villa — the villa stripped back to structure during the works"
    - src: ../../../assets/projects/la-villa-nice/055-img-9182.jpg
      alt: "La Villa — the villa stripped back to structure during the works"
    - src: ../../../assets/projects/la-villa-nice/053-img-8892.jpg
      alt: "La Villa — the villa stripped back to structure during the works"
---

A private residence organised around a single decision: let the stone speak and keep everything else quiet.

Full-height slabs of brown onyx and Calacatta marble set the tone in the bathrooms and the kitchen. Against them the joinery stays matte and pale, the panelling keeps its classical mouldings, and brass appears only where a hand actually lands — mixers, sconces, the ribbon of the chandelier over the dining table.

The house came to the studio as a shell: pink plaster, exposed services, rubble on the floors. The open oak stair is the one sculptural gesture kept from the original plan.
