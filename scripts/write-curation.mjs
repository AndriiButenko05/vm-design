/**
 * Перетворює відбір за індексами контактних аркушів на src/data/curation.json.
 *
 *   node scripts/write-curation.mjs
 *
 * Індекси беруться з аркушів у scratchpad (підпис під кожним кадром).
 * Зберігати відбір індексами зручніше, ніж іменами файлів: аркуш перед очима,
 * а імена на кшталт "046-img-2451.jpg" не читаються.
 *
 * Розділяємо два потоки:
 *   after  — опубліковані кадри готового інтер'єру, перший = обкладинка
 *   before — стан до робіт, для блоку «до / після»
 *
 * Групи, яких тут немає, лишаються без курації: prepare-images візьме всі кадри.
 */
...
/** Індекси з контактних аркушів. Перший у `after` стає обкладинкою. */
const PICKS = {
  'christina-kassel': {
  ...
  'christina-roma': {
    // 31 — вітрина після робіт. Знята з тієї самої точки, що й 4 «до»,
    // тому саме ця пара йде в режим slider.
  ...
  // Кадри 28–29 у теці Paris — простір бренду DARPHIN, 30–33 — лаунж
  // незрозумілої належності. До Christina Paris не включаю, поки не уточнимо.
  'christina-paris': {
    after: [5, 22, 3, 11, 6, 16, 14, 9, 20, 23, 17, 15, 27, 25],
  },

  // Кадри 00–03 — інший бренд, DERMADIANE. Теж винесено за дужки.
  'christina-monaco': {
...
/**
 * Індекси рахуються від СИРОЇ теки, а не від маніфесту: маніфест після
 * першої ж курації містить лише відібране, і нумерація в ньому зсувається.
 * Сира тека — та сама основа, за якою збиралися контактні аркуші.
 */
